...
const WeatherConditionsDashboard = ({ beachId }) => {
    ...
    try {
      ...
      const formattedData = data.data[0].beachmetric
        .map((item) => ({
          ...
          windDirection: item.wind?.direction || null,
          cloudCoverage: item.cloud_coverage || null,
        }))
        .filter(item =>
          item.windSpeed !== null ||
          item.windDirection !== null ||
          item.cloudCoverage !== null
        );
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="border rounded-lg p-4 bg-white shadow">
            <h2 className="text-xl font-bold mb-4">Weather Conditions Chart</h2>
            <div className="h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart
                  data={formatDataForChart(weatherData)}
                ...
                >
                  ...
                  <XAxis
                    ...
                    textAnchor="end"
                    height={80}
                    interval={Math.ceil(weatherData.length / 10)}
                    tick={{ fontSize: 12 }}
                  />
                  <YAxis
                    yAxisId="left"
              ...
              <table className="w-full border-collapse">
                <thead className="sticky top-0 bg-white">
                  <tr>
                    ...
                    <th className="border p-2 text-left">Wind Speed (km/h)</th>
                    <th className="border p-2 text-left">Wind Direction (°)</th>
                    <th className="border p-2 text-left">Cloud Coverage (%)</th>
                  </tr>
                </thead>
                <tbody>
                  {weatherData.map((item, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="border p-2">
                        {item.windSpeed?.toFixed(1) ?? 'N/A'}
                      </td>
                      <td className="border p-2">
                        {item.windDirection ?? 'N/A'}
                      </td>
                      <td className="border p-2">
                        {item.cloudCoverage?.toFixed(1) ?? 'N/A'}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
export default WeatherConditionsDashboard;