...
const WaterConditionsDashboard = ({ beachId }) => {
  ...
  const generateDailyReference = () => {
    ...
    setTideHeight(levels[Math.floor(Math.random() * levels.length)]);
  };
  ...
  const fetchBeachData = async () => {
    try {
      ...
      const formattedData = data.data[0].beachmetric
        ...
        .filter(item => item.waveHeight !== null || item.waterTemperature !== null);
      setWaterData(formattedData);
      generateDailyReference();
    } catch (error) {
  ...
  return (
    <div className="p-4 space-y-4">
      ...
      {loading ? (
        ...
        <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="border rounded-lg p-4 bg-white shadow">
            <h2 className="text-xl font-bold mb-4">Water Conditions Chart</h2>
            <div className="h-[400px]">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart 
                  data={formatDataForChart(waterData)} 
                ...
                >
                  ...
                  <XAxis 
                    ...
                    textAnchor="end"
                    height={80}
                    interval={Math.ceil(waterData.length / 10)}
                    tick={{ fontSize: 12 }}
                  />
                  <YAxis 
                    yAxisId="left" 
              ...
              <table className="w-full border-collapse">
                <thead className="sticky top-0 bg-white">
                  <tr>
                    ...
                    <th className="border p-2 text-left">Wave Height (m)</th>
                ...
                <tbody>
                  {waterData.map((item, index) => (
                    <tr key={index} className="hover:bg-gray-50">
                      ...
                      <td className="border p-2">
                        {item.waveHeight?.toFixed(2) ?? 'N/A'}
                      </td>
                      <td className="border p-2">
                        {item.waterTemperature?.toFixed(1) ?? 'N/A'}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <div className="text-center mt-4">
        <p className="text-lg font-bold">Current Tide Level: {tideHeight}</p>
      </div>
    </div>
  );
};

export default WaterConditionsDashboard;